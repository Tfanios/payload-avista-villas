...
enquiries**. Keep it a _personal-enquiry_ model, **not** a booking engine (see `PRODUCT.md`
...
_(If you prefer to keep images static for v1, skip this and store image paths as `text` — but you lose
the responsive sizes and the 100-score is much harder. Recommended: use the collection.)_
...
export const heroMedia: Field[] = [
  ...
  },
  {
    ...
    relationTo: 'media',
    admin: {
      description: 'Portrait-friendly crop shown <768px. Falls back to imageDesktop if empty.',
    },
  },
  ...
  // --- Data-saver + LCP controls ---
  {
    ...
    admin: {
      description:
        'OFF = phones show the poster image only and never download the video (recommended for the home hero).',
    },
  },
  {
    name: 'priority',
    ...
    admin: {
      description:
        'Above-the-fold hero: front end preloads it + sets fetchpriority="high". Turn OFF for heroes below the fold.',
    },
  },
]
```
...
export const mapLocation: Field[] = [
  {
    ...
    type: 'number',
    required: true,
    admin: { description: 'Pin latitude, e.g. 40.1969' },
  },
  {
    name: 'longitude',
    type: 'number',
    required: true,
    admin: { description: 'Pin longitude, e.g. 23.7761' },
  },
  { name: 'zoom', type: 'number', defaultValue: 14, min: 1, max: 20 },
  {
    name: 'label',
    type: 'text',
    admin: { description: 'Pin tooltip / place name, e.g. "Avista Villa, Vourvourou"' },
  },
  {
  ...
  },
  {
    ...
    admin: {
      description:
        'Static map image (with the pin baked in) shown as the facade. The interactive iframe only loads on click — keeps the map off the critical path.',
    },
  },
]
```
      ...
      async ({ doc, req, operation }) => {
        ...
        try {
          ...
          await req.payload.update({
            collection: 'media',
            id: doc.id,
            data: { blurDataURL: doc.blurDataURL },
          })
        } catch {
          /* non-fatal: image still works without the placeholder */
        }
        return doc
      },
    ],
